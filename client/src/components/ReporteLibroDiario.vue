...
const props = defineProps({
  ...
  reportData: Array // { Ncomp, Linea, Fecha, Cuenta, Debe, Haber }
})

const emit = defineEmits(['close'])

// Agrupa por Ncomp (número de póliza/comprobante), igual que el Crystal Report original
const groupedData = computed(() => {
  if (!props.reportData || props.reportData.length === 0) return {}

  return props.reportData.reduce((acc, row) => {
    const key = row.Ncomp || 'SIN POLIZA'
    if (!acc[key]) {
      acc[key] = { items: [], subtotalDebe: 0, subtotalHaber: 0 }
    }
  ...
  <div v-if="show" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
      ...
      <div class="no-print bg-slate-800 border-b border-slate-700 px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <span class="text-xs text-slate-400 font-mono">Vista Previa - Libro Diario</span>
      </div>

      <!-- Document Sheet: el scroll vive en el "papel" blanco (max 24 líneas +
           sus 2 totales de póliza), no en este contenedor oscuro -- por eso ya
           no lleva flex-1 (que lo estiraba a llenar el h-[90vh] fijo del modal,
           dejando espacio vacío azul debajo del papel). -->
        ...
        <div class="printable-area shrink-0 overflow-y-auto max-h-[65vh] bg-white text-black p-10 shadow-2xl w-[210mm] font-sans text-[11px]">
          ...
          <div>
            <div class="text-center mb-6">
              ...
              <h2 class="text-sm font-bold mt-0.5">Libro Diario</h2>
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody>
                <template v-for="(group, ncomp) in groupedData" :key="ncomp">
                  <tr v-for="item in group.items" :key="ncomp + '-' + item.Linea" class="border-b border-gray-100">
                    <td class="py-0.5 whitespace-nowrap">{{ item.Ncomp }}</td>
                    <td class="py-0.5">{{ item.Cuenta }}</td>
                  ...
                  <!-- Subtotal por póliza (Ncomp) -->
                  <tr class="font-bold border-t border-black border-b">
                    <td colspan="4" class="py-1 text-right pr-2">Total Póliza {{ ncomp }}:</td>
                  ...
                  <!-- Salto de línea extra entre pólizas -->
                  <tr class="h-3"><td colspan="6"></td></tr>
                </template>

                <!-- Total General final -->
                <tr v-if="reportData && reportData.length > 0" class="font-bold border-t-2 border-b-2 border-black">
                  <td colspan="4" class="py-2 text-right pr-2 uppercase">Total General:</td>
